...
import fr.pantheosorbonne.cri.HLAdaptor;
...
@Component(immediate = true, service = ReactiveForwarding.class, property = {
        ...
        FLOW_TIMEOUT + ":Integer=" + FLOW_TIMEOUT_DEFAULT, FLOW_PRIORITY + ":Integer=" + FLOW_PRIORITY_DEFAULT,
...
public class ReactiveForwarding {
    ...
    public void activate(ComponentContext context) {
        ...
        HLAdaptor facade = HLAdaptor.getDefaultBuilder().withDeviceService(deviceService)//
        ...
        KryoNamespace.Builder metricSerializer = KryoNamespace.newBuilder().register(KryoNamespaces.API)
                .register(ReactiveForwardMetrics.class).register(MultiValuedTimestamp.class);
        metrics = storageService.<MacAddress, ReactiveForwardMetrics>eventuallyConsistentMapBuilder()
                .withName("metrics-fwd").withSerializer(metricSerializer)
                .withTimestampProvider(
                        (key, metricsData) -> new MultiValuedTimestamp<>(new WallClockTimestamp(), System.nanoTime()))
                .build();

        blackHoleExecutor = newSingleThreadExecutor(groupedThreads("onos/app/fwd", "black-hole-fixer", log));
            ...
            public void run() {
                ...
                while (true) {
                    ...
                    try (BufferedReader reader = new BufferedReader(new FileReader("/home/nherbaut/intent.txt"))) {
                        ...
                        newlyAllowedHosts.stream().peek(h -> logToFile("unblock " + h))
                                .peek(h -> blockingRule(h, null, true)).forEach(h -> blockingRule(null, h, true));
    ...
    private void readComponentConfiguration(ComponentContext context) {
        ...
        if (matchIcmpFieldsEnabled == null) {
            log.info("Matching ICMP (v4 and v6) fields is not configured, " + "using current value of {}",
                    matchIcmpFields);
        } else {
        ...
        if (ignoreIpv4McastPacketsEnabled == null) {
            log.info("Ignore IPv4 multi-cast packet is not configured, " + "using current value of {}",
                    ignoreIPv4Multicast);
        } else {
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
            ...
            // NHE
            if ((blockedHosts.contains(HostId.hostId(ethPkt.getDestinationMAC().toString() + "/None"))
                    || blockedHosts.contains(HostId.hostId(macAddress.toString() + "/None")))) {
                droppedPacket(macMetrics);
                log.info("packet drop due to policy restriction");
                return;
            ...
            if (path == null) {
                log.warn("Don't know where to go from here {} for {} -> {}", pkt.receivedFrom(), ethPkt.getSourceMAC(),
        ...
        private void removeDropAllToHost(HostId hostId) {
            FilteringObjective.Builder filteringObjectiveBuilder = DefaultFilteringObjective.builder().deny()
                    ...
                    .withPriority(50000).addCondition(Criteria.matchEthDst(hostId.mac()));
        ...
        private void dropAllToHost(HostId hostId) {
            ...
            FilteringObjective.Builder filteringObjectiveBuilder = DefaultFilteringObjective.builder().deny()
                    .fromApp(appId).withPriority(50000).addCondition(Criteria.matchEthDst(hostId.mac()));
