    ...
    public void activate() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while (true) {
                    ...
                    try (BufferedReader reader = new BufferedReader(new FileReader("/home/nherbaut/intent.txt"))) {
                        ...
                        Set<HostId> newlyAllowedHosts = blockedHosts.stream().peek(l -> log.warn("block " + l.toString())).filter(l -> !lines.contains("block " + l.toString())).collect(Collectors.toSet());
            ...
            private void removeIntentsForHost(HostId hostId) {
                ...
                intentsToDiscard.parallelStream().peek(i -> log.warn("removing intent from {} to {}", i.one(),i.two())).forEach(intent -> intentService.withdraw(intent));
